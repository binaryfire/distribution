...
func testUploadFS(t *testing.T, numUploads int, repoName string, startedAt time.Time) (driver.StorageDriver, context.Context) {
	...
	for i := 0; i < numUploads; i++ {
		addUploads(ctx, t, d, dcontext.NewUUIDString(), repoName, startedAt)
	}
	return d, ctx
}
...
func TestPurgeAll(t *testing.T) {
	...
	addUploads(ctx, t, fs, dcontext.NewUUIDString(), "test-repo2", oneHourAgo)
	uploadCount++
...
func TestPurgeSome(t *testing.T) {
	...
	for i := 0; i < newUploadCount; i++ {
		addUploads(ctx, t, fs, dcontext.NewUUIDString(), "test-repo", time.Now().Add(1*time.Hour))
	}
...
func TestPurgeOnlyUploads(t *testing.T) {
	...
	dataPath, err := pathFor(uploadDataPathSpec{name: "test-repo", id: dcontext.NewUUIDString()})
	if err != nil {
		t.Fatal(err)
	}
